...
if Areas is None:
    print("âŒ CRITICAL ERROR: Could not find 'Area' Enum.")
    sys.exit(1)
# =====================================================
...
def _load_env(path: str = '.env') -> None:
    ...
    if not os.path.exists(path):
        print(f"âš ï¸  No .env file found at '{path}'. Using OS environment variables.")
        return
...
# ================= CSV -> READ PLAN ===================
from ems_tag_parser import load_csv_tags as _load_csv_tags

# S7 area values used by ems_tag_parser:
#   DB=132, Inputs=129, Outputs=130, Markers=131
# =====================================================

def _chunk_tags(group_tags: list, pdu_max: int) -> list:
    """
    Split a list of tags (all in the same DB) into PDU-sized read chunks.
    Each chunk covers a contiguous byte range â‰¤ pdu_max bytes.
    ...
    for tag in sorted_tags:
        ...
        if chunk_start is None:
            ...
            # Current tag doesn't fit â€” flush the current chunk and start a new one
            chunk_end = max(
...
def build_read_plans(csv_files: list) -> list:
    ...
    for csv_file in csv_files:
        ...
        print(f"   ðŸ“„ {csv_file}: {len(file_tags)} tags loaded.")
        all_tags.extend(file_tags)

    if not all_tags:
        print("âŒ No tags loaded from any CSV file. Exiting.")
        sys.exit(1)
    ...
    print(f"âœ… Total: {len(all_tags)} tags across {len(plans)} DB block(s), "
...
def main():
    print("=" * 55)
    print(f"  EMS Edge â€” {EQUIPMENT_NAME} ({EQUIPMENT_CODE})")
    ...
    try:
        ...
        print(f"âŒ MQTT Error: {e}")
        return

    # --- PLC Setup ---
    plc = snap7.client.Client()
    try:
        print(f"â³ Connecting to PLC {PLC_IP} (rack={PLC_RACK}, slot={PLC_SLOT})...")
        plc.connect(PLC_IP, PLC_RACK, PLC_SLOT)
        print("âœ… PLC Connected.")
    except Exception as e:
        print(f"âŒ PLC Error: {e}")
        return

    # --- PLC Diagnostics ---
    print("ðŸ” PLC Diagnostics:")
    try:
        ...
        print(f"   âŒ get_cpu_info failed: {type(e).__name__}: {e}")
        print("   âš ï¸  This usually means PUT/GET access is disabled in PLC properties.")
    ...
    if read_plans:
        ...
        try:
            ...
            print(f"   âœ… Test read DB{first_plan['db']} offset {first_chunk['start']} â†’ OK")
        except Exception as e:
            err_str = str(e)
            print(f"   âŒ Test read DB{first_plan['db']} failed: {type(e).__name__}: {err_str}")
            if 'refused' in err_str.lower() or '0x00200000' in err_str:
                print("")
                print("   â•”â•â• FIX REQUIRED (PLC-side) â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—")
                print("   â•‘  Error 0x00200000 = 'Function refused by CPU'           â•‘")
                print("   â•‘  Most likely causes (in order):                         â•‘")
                print("   â•‘  1. S7-1200/1500: PUT/GET access NOT enabled            â•‘")
                print("   â•‘     â†’ TIA Portal â†’ PLC Properties â†’ Protection         â•‘")
                print("   â•‘       â†’ âœ… 'Permit access with PUT/GET...'              â•‘")
                print("   â•‘  2. S7-1200/1500: DB102 has 'Optimized block access'    â•‘")
                print("   â•‘     â†’ TIA Portal â†’ DB102 Properties                    â•‘")
                print("   â•‘       â†’ âœ… UN-check 'Optimized block access'            â•‘")
                print("   â•‘  3. S7-1200: Wrong slot â€” try PLC_SLOT=1 in .env       â•‘")
    ...
    print(f"ðŸš€ Starting loop â€” {BURST_SAMPLES} burst reads / {CYCLE_TIME}s cycle...")
                    ...
                    for chunk in plan['chunks']:
                        try:
                            ...
                            if main._logged_errs.get(err_key, 0) < 3:
                                ...
                                print(f"   âš ï¸  DB{plan['db']} chunk@{chunk['start']}: "
                                      f"{type(e).__name__}: {e}")
